...
class VQ_Critic(nn.Module):
    ...
    def score_matrix_log(self, score): # score (b, 1024)
        for i in range(score.shape[0]):
            ...
            wandb.log({f"{i:02d}_score_matrix": wandb.Image(fig)})
            plt.close()
    
    def attn_matrix_log(self, score): # score (b, 1024)
        for i in range(score.shape[0]):
            score_2d = score[i].view(32, 32)
            df = pd.DataFrame(score_2d.to('cpu').numpy())
            fig, ax = plt.subplots(figsize = (20, 20))
    ...
    def score_matrix_log_tc(self, score): # score (b, 1024)
        for i in range(score.shape[0]):
            ...
            fig, ax = plt.subplots(figsize = (20, 20))
            sns.heatmap(df, annot=df, fmt='.2f', cbar=False, vmin=0, vmax=1, cmap="Greys")
            wandb.log({f"{i:02d}_score_matrix_tc": wandb.Image(fig)})
            plt.close()
    ...
    def mask_token_log(self, out_idx): # out2_idx (b, 1024 with mask token)
        for i in range(out_idx.shape[0]):
            ...
            wandb.log({f"{i:02d}_content": wandb.Image(fig)})   
            plt.close() 
    ...
    def mask_token_log_tc(self, out_idx): # out2_idx (b, 1024 with mask token)
        for i in range(out_idx.shape[0]):
            ...
            fig, ax = plt.subplots(figsize = (20, 20))
            sns.heatmap(df, annot=an, cbar=False, fmt = '', vmin = -(self.num_classes - 1), vmax = self.num_classes - 1)
            wandb.log({f"{i:02d}_content_tc": wandb.Image(fig)})   
            plt.close()
    ...
    @torch.no_grad()
    def inference_generate_sample_with_condition(self, text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, wandb_log="s,t,r", a=0., b=0., score_weight = 1., attn_weight = None, token_critic=True, gaussian_alpha = 1.):
        """
        step: 16, 50, 100
    ...
    @torch.no_grad()
    def inpaint_test(self, text, text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, noise_step=5, noise_power=5 ,wandb_log="s,t,r", a=0., b=0., score_weight=1., purity=True, gaussian_alpha = 2.):
        """
        step: 16, 50, 100
    ...
    @torch.no_grad()
    def ediff_test(self, text, text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, wandb_log="s,t,r", a=0., b=0., score_weight=1., purity=False, start_step = 8):
        log_set = wandb_log.split(',')
        save_root = 'RESULT'
        os.makedirs(save_root, exist_ok=True)
        str_cond = str(text) + '_' + str(step)
        ...
        with torch.no_grad():
            # initialize with purity or random revoke
            for i, (n, diffusion_index) in tqdm(enumerate(zip(n_sample[:start_step-1], time_list[:start_step-1]))): # before last step
                t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                _, log_x_recon = self.VQ_Diffusion.model.transformer.p_pred(log_z, cond_emb, t)
                out = self.VQ_Diffusion.model.transformer.log_sample_categorical(log_x_recon) # recon -> sample -> x_0
                out_idx = log_onehot_to_index(out) # b, 1024
                ...
                # else:
                print(f"Random initialize i = {i} with text1")
                for ii in range(batch_size):
                    sel = torch.multinomial(score[ii], n)
                    out2_idx[ii][sel] = out_idx[ii][sel]
                log_z = index_to_log_onehot(out2_idx, self.num_classes)
            ...
            print(f"Random initialize i = {start_step-1} with text1")
            t = torch.full((batch_size,), time_list[start_step], device=device, dtype=torch.long)
            _, log_x_recon = self.VQ_Diffusion.model.transformer.p_pred(log_z, cond_emb, t)
            out = self.VQ_Diffusion.model.transformer.log_sample_categorical(log_x_recon) # recon -> sample -> x_0
            out_idx = log_onehot_to_index(out) # b, 1024
            self.recon_image_log(out_idx)
            ...
            for i, (n, diffusion_index) in tqdm(enumerate(zip(n_sample[start_step:], time_list[start_step:]))):
                # random noise
                print(f"with text2 i = {i+start_step}")
                out2_idx = torch.full_like(out_idx, self.num_classes-1).to(out_idx.device)
                score = torch.ones_like(out_idx).float().cuda()
                for ii in range(batch_size):
                    sel = torch.multinomial(score[ii], n)
                    out2_idx[ii][sel] = out_idx[ii][sel]
                log_z = index_to_log_onehot(out2_idx, self.num_classes) # noise added
...
if __name__ == '__main__':
    ...
    #     wandb.finish()
    # ediff_test(text, text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, noise_step=5, noise_power=5 ,wandb_log="s,t,r", a=0., b=0., score_weight=1., purity=False, start_step = 8):
    # wandb.init(project='1116', name = '789_10_attn_in_TC')
    # wandb.finish()
    # text = 'A picture of a teddy bear on a stone.'
    # wandb.init(project='1116', name = text)
    # wandb.finish()

    # model.accuracy_test()

    # wandb.init(project='1116', name='halfTC_attn5_tc-gNone_alpha1')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 5, step=16, wandb_log='r', gaussian_alpha = 1.)
    # wandb.finish()

    # wandb.init(project='1116', name='halfTC_attn5_tc-gNone_alpha2')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 5, step=16, wandb_log='r', gaussian_alpha = 2.)
    # wandb.finish()

    # wandb.init(project='1116', name='halfTC_attn3_tc-gNone_alpha3')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 3, step=16, wandb_log='r', gaussian_alpha = 3.)
    # wandb.finish()

    # wandb.init(project='1116', name='halfTC_attn5_tc-gNone_alpha3')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 5, step=16, wandb_log='r', gaussian_alpha = 3)
    # wandb.finish()

    # # tc guidance

    # wandb.init(project='1116', name='halfTC_attn5_tc-g3_alpha3')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=3., attn_weight = 5, step=16, wandb_log='r', gaussian_alpha = 3.)
    # wandb.finish()

    # wandb.init(project='1116', name='halfTC_attn5_tc-g5_alpha4')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=5., attn_weight = 5, step=16, wandb_log='r', gaussian_alpha = 4.)
    # wandb.finish()

    # wandb.init(project='1116', name='halfTC_attn3_tc-g3_alpha3')
    # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=3., attn_weight = 3, step=16, wandb_log='r', gaussian_alpha = 3.)
    # wandb.finish()
    ...
    # wandb.init(project='1116', name='RR_attn_1')
    # model.inference_generate_sample_with_condition(text='a black honda motorcycle parked in front of a garage.', batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 1, step=16, wandb_log='r,a', token_critic=False)
    # wandb.finish()

    # wandb.init(project='1116', name='RR_attn_3')
    # model.inference_generate_sample_with_condition(text='a black honda motorcycle parked in front of a garage.', batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 5, step=16, wandb_log='r,a', token_critic=False)
    # wandb.finish()

    # wandb.init(project='1116', name='RR_attn_5')
    # model.inference_generate_sample_with_condition(text='a black honda motorcycle parked in front of a garage.', batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, attn_weight = 5, step=16, wandb_log='r,a', token_critic=False)
    # wandb.finish()

    # wandb.init(project='1116', name='789_10_attn_a1')
    # model.inference_generate_sample_with_condition(text="teddy bear playing in the pool", batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, wandb_log='r,a', gaussian_alpha = 1)
    # wandb.finish()

    # wandb.init(project='1116', name='789_10_attn_a2')
    # model.inference_generate_sample_with_condition(text="teddy bear playing in the pool", batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=16, wandb_log='r,a', gaussian_alpha = 2)
    # wandb.finish()
    ...
    # wandb.init(project='1128', name = '16_00')
    ...
    # wandb.init(project='1128', name = '16_00_3')
    ...
    # # text = 'a car that seems to be parked illegally behind a legally parked car'
        ...
        # wandb.init(project='1128', name = '16_00_5_a2')
        # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=5, step=16, wandb_log='',a=0, b=0, gaussian_alpha = 2., attn_weight = 3)
        # wandb.finish()

        # wandb.init(project='1128', name = '16_00_5_a3')
        # model.inference_generate_sample_with_condition(text=text, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=5, step=16, wandb_log='',a=0, b=0, gaussian_alpha = 3., attn_weight = 3)
        # wandb.finish()



    # wandb.init(project='1128', name = '16_105_5')
    ...
    # wandb.init(project='1128', name = '16_00_9')
    ...
    # wandb.init(project='1128', name = '50_00')
    ...
    # wandb.init(project='1128', name = '100_00')
    ...
    # wandb.init(project='1128', name = 'refine_16step_5start_weight1_purity')
    ...
    # wandb.init(project='1128', name = 'refine_16step_8start_weight1_purity')
    ...
    # wandb.init(project='1128', name = 'refine_16step_11start_weight1_purity')
    ...
    # wandb.init(project='1128', name = 'refine_50step_15start_weight1_purity')
    ...
    # wandb.init(project='1128', name = 'refine_50step_25start_weight1_purity')
    ...
    # wandb.init(project='1128', name = 'refine_50step_35start_weight1_purity')
    ...
    # # # # 16
    # wandb.init(project='1128', name = 'draft_16step_10_5_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_16step_10_7_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_16step_10_9_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_10_15_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_10_20_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_10_30_weight1_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_2_25_weight1_g0_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_2_25_weight1_g3_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_2_25_weight1_g5_r')
    ...
    # wandb.init(project='1128', name = 'draft_50step_2_25_weight1_g9_r')
    ...
    # wandb.init(project='1128', name = text + ' ' + text2)
    # wandb.finish()

    # wandb.init(project='1128', name = 'paint_50step_15_25_weight3_g0_r')
    # model.inpaint_test(text=text, text2 = text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=50, noise_step=15, noise_power=25, wandb_log='r,s', a=0, b=0, score_weight=1., purity=False)
    # wandb.finish()

    # wandb.init(project='1128', name = 'paint_50step_15_40_weight1_g0_r')
    # model.inpaint_test(text=text, text2 = text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=None, step=50, noise_step=15, noise_power=40, wandb_log='r,s', a=0, b=0, score_weight=1., purity=False)
    # wandb.finish()

    # wandb.init(project='1128', name = 'paint_50step_15_40_weight1_g3_r')
    # model.inpaint_test(text=text, text2 = text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=3., step=50, noise_step=15, noise_power=40, wandb_log='r,s', a=0, b=0, score_weight=1., purity=False)
    # wandb.finish()

    # wandb.init(project='1128', name = 'paint_50step_15_40_weight1_g5_r')
    # model.inpaint_test(text=text, text2 = text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=5., step=50, noise_step=15, noise_power=40, wandb_log='r,s', a=0, b=0, score_weight=1., purity=False)
    # wandb.finish()

    # wandb.init(project='1128', name = 'paint_50step_15_40_weight1_g2_r')
    # model.inpaint_test(text=text, text2 = text2, batch_size=4, vq_guidance=5.0, vq_tr=0.86, tc_guidance=9., step=50, noise_step=15, noise_power=40, wandb_log='r,s', a=0, b=0, score_weight=1., purity=False)
    # wandb.finish()
