...
from image_synthesis.engine.token_critic_solver2 import Token_Critic_Solver
...
def get_args():
    ...
    parser.add_argument('--tc_det', type=bool, default=False, 
                        help='if it is True, deterministic sampling (default: False)')
    parser.add_argument('--tc_alpha', type=float, default=1., 
                        help='deterministic gaussian noise prob scale (default: 1.0) ')
...
def main():
    args = get_args()
    args.vq_path = diffusion_model_path
...
def main_worker(local_rank, args):
    ...
    vq_config = load_yaml_config('configs/coco_tune.yaml')
    VQ_Diffusion_model = build_model(vq_config, args)

    Token_Critic_model = Token_Critic(config=config, learnable_cf=False)
    # Token_Critic_model.empty_text_embed = torch.load('cf_emb.pth').requires_grad_(True)
