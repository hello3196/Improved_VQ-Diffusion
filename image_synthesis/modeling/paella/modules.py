...
class DenoiseUNet(nn.Module):
    def __init__(self, num_labels, c_hidden=960, c_clip=512, c_r=64, down_levels=[4, 8, 16], up_levels=[16, 8, 4]):
        super().__init__()
        ...
        self.clf = nn.Conv2d(c_levels[0], 1, kernel_size=1)
        self.logit_sigmoid = nn.Sigmoid()
    ...
    def forward(self, x, c, r):  # r is a uniform value between 0 and 1
        ...
        x = self.clf(x)
        x = self.logit_sigmoid(x)
        return x


if __name__ == '__main__':
    device = "cuda"
    model = DenoiseUNet(num_labels=2887, c_hidden=640, c_clip=512, c_r=64, down_levels=[4, 8, 16], up_levels=[16, 8, 4]).to(device)
    print(sum([p.numel() for p in model.parameters()]))
    x = torch.randint(0, 2887, (4, 32, 32)).long().to(device)
    c = torch.randn((4, 512)).to(device)
    r = torch.rand(4).to(device)
    print(f"model(x, c, r) shape: {model(x, c, r).shape}")
